...
def input_train():
  ...
  x = tf.reshape(batch[0], [-1, 28, 28, 1])
  return x, batch[1]
...
def input_test():
  ...
  """
  test_x = tf.reshape(mnist.test.images, [-1, 28, 28, 1])
...
def main(argv=None):
  ...
  x = tf.placeholder(tf.float32, [-1, x_width, x_height, x_channel])
  y_ = tf.placeholder(tf.float32, [None, class_num])
  keep_prob = tf.placeholder(tf.float32)

  with tf.name_scope('Converlution Layer 1'):
  ...
  with tf.name_scope('Training'):
    learning_rate = 1e-4
    training_steps = 1000
    loss_corss_entropy = tf.reduce_mean(tf.reduce_sum(-y_ * tf.log(y_predict)))
